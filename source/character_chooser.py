...
class CharacterChooser:
	...
	def load_character(self):
		...
		if self.register.save_character(self.user, self.selected_character):
			...
			self.window.destroy()
			# he acabado aqui, TODO: llamar a la siguiente ventana
		else:
	...
	def show_interface(self):
		...
		nombre_label = tk.Label(self.window, text="Nombre del personaje: ", font=("Arial", 16))
		...
		for i, imagen in enumerate(personajes.keys()):
			...
			boton = tk.Button(self.window, image=img_tk, command=lambda imagen=imagen: self.seleccionar_personaje(imagen, personajes, ataques_entries, nombre_entry))
			boton.place(x=20 + i*200, y=200)

		ataques_label = tk.Label(self.window, text="Ataques del personaje: ", font=("Arial", 16))
		ataques_label.place(x=20, y=440)

		ataques_entries = [tk.Entry(self.window, bg='#333333', fg="white", font=("Arial", 16)) for _ in range(4)]
		for i, ataque_entry in enumerate(ataques_entries):
			if i % 2 == 0:
				ataque_entry.place(x=20, y=480 + i*20)
			else:
				ataque_entry.place(x=400, y=480 + (i-1)*20)

		continuar_button = tk.Button(self.window, text="Finalizar", bg="#FF3399", fg="#FFFFFF", font=("Arial", 26), command=self.load_character)
		continuar_button.place(x=520, y=100)
